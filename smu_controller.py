...
class SMUController(BaseInstrumentController):
    ...
    def __init__(self, address: str, name: str = "SMU", mock: bool = False, channel: int = 1):
        ...
        # Channel support (1 or 2 for dual-channel SMUs like B2902A)
        if channel not in [1, 2]:
            raise ValueError(f"Invalid channel: {channel}. Must be 1 or 2.")
        self.channel = channel
        self._ch = f"(@{channel})"  # SCPI channel suffix
    ...
    def _scpi(self, cmd: str) -> str:
        """Format SCPI command with channel suffix if applicable."""
        if self._ch:
            return f"{cmd},{self._ch}"
        return cmd
    ...
    def connect(self) -> None:
        ...
        try:
            ...
            # Auto-detect single vs dual channel SMU
            # B2901A = single channel, B2902A = dual channel
            if "B2901" in idn:
                self._is_single_channel = True
                if self.channel != 1:
                    self.handle_error(f"B2901A is single-channel. Channel {self.channel} not available.")
                    return
                # Single-channel SMUs don't need channel suffix
                self._ch = ""
                self.logger.info("Detected single-channel SMU (B2901A), disabling channel suffix")
            elif "B2902" in idn:
                self._is_single_channel = False
                self.logger.info(f"Detected dual-channel SMU (B2902A), using channel {self.channel}")
            else:
                # Unknown model - keep channel suffix for safety
                self._is_single_channel = False
                self.logger.warning(f"Unknown SMU model, assuming dual-channel support")
            
            # Reset to known state
            self.resource.write("*RST")
            if self._ch:
                self.resource.write(f"SOUR:FUNC:MODE CURR,{self._ch}")  # Current source mode
            else:
                self.resource.write("SOUR:FUNC:MODE CURR")  # Single-channel
    ...
    def set_compliance(self, limit: float, limit_type: str) -> None:
        ...
        try:
            ...
            self.resource.write(self._scpi(f"SENS:{limit_type}:PROT {limit}"))
        except Exception as e:
    ...
    def set_nplc(self, nplc: float) -> None:
        ...
        try:
            ...
            self.resource.write(self._scpi(f"SENS:VOLT:NPLC {nplc}"))
            self.resource.write(self._scpi(f"SENS:CURR:NPLC {nplc}"))
        except Exception as e:
    ...
    def set_current(self, amps: float) -> None:
        ...
        try:
            self._check_current_limit(amps)
            self.resource.write(self._scpi("SOUR:FUNC:MODE CURR")) 
            self.resource.write(self._scpi(f"SOUR:CURR {amps}"))
            self._current_source_amps = amps
    ...
    def set_source_mode(self, mode: str) -> None:
        ...
        try:
            self.resource.write(self._scpi(f"SOUR:FUNC:MODE {mode}"))
    ...
    def set_voltage(self, volts: float) -> None:
        ...
        try:
            self.resource.write(self._scpi(f"SOUR:VOLT {volts}"))
        except Exception as e:
    ...
    def enable_output(self) -> None:
        ...
        try:
            self.resource.write(self._scpi("OUTP ON"))
            self._output_enabled = True
            # Double check
            query_cmd = f"OUTP? {self._ch}" if self._ch else "OUTP?"
            if not self.mock and "1" not in self.resource.query(query_cmd):
                 self.logger.warning(f"SMU Channel {self.channel} did not report Output ON after command!")
            self.to_state(InstrumentState.RUNNING)
        except Exception as e:
    ...
    def disable_output(self) -> None:
        ...
        try:
            ...
            self.resource.write(self._scpi("OUTP OFF"))
            self._output_enabled = False
            self.to_state(InstrumentState.IDLE)
        except Exception as e:
    ...
    def setup_list_sweep(self, points: list[float], source_mode: str, time_per_step: float, trigger_count: int = 1) -> None:
        ...
        try:
             # 1. Select Function
            self.resource.write(self._scpi(f"SOUR:FUNC:MODE {source_mode}"))
            
            # 2. Set Mode to LIST
            self.resource.write(self._scpi(f"SOUR:{source_mode}:MODE LIST"))
            ...
            self.resource.write(self._scpi(f"SOUR:LIST:{source_mode} {points_str}"))
            
            # 4. Timing Config (Timer Trigger)
            # Source Trigger: Timer
            self.resource.write(self._scpi("TRIG:TRAN:SOUR TIM"))
            self.resource.write(self._scpi(f"TRIG:TRAN:TIM {time_per_step}"))
            
            # 5. Length of sequence
            self.resource.write(self._scpi(f"TRIG:TRAN:COUN {len(points)}"))
            
            # 6. Repetitions
            self.resource.write(self._scpi(f"ARM:TRAN:COUN {trigger_count}"))
    ...
    def trigger_list(self):
        ...
        try:
            ...
            init_cmd = f"INIT {self._ch}" if self._ch else "INIT"
            self.resource.write(init_cmd)  # Init configured channel
            self.to_state(InstrumentState.RUNNING)
        except Exception as e:
    ...
    def measure(self) -> Dict[str, float]:
        ...
        try:
            ...
            v_query = f"MEAS:VOLT? {self._ch}" if self._ch else "MEAS:VOLT?"
            i_query = f"MEAS:CURR? {self._ch}" if self._ch else "MEAS:CURR?"
            
            v_str = self.resource.query(v_query).strip()
            i_str = self.resource.query(i_query).strip()
